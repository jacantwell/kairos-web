  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        try {
          const journeyResponse = await api.users.getActiveJourneyApiV1UsersUserIdJourneysActiveGet(userId);
          
          if (journeyResponse.status === 200 && journeyResponse.data) {
            setActiveJourney(journeyResponse.data);
            
            // If there's an active journey, fetch its markers
            if (journeyResponse.data._id) {
              const markersResponse = await api.journeys.getJourneyMarkersApiV1JourneysJourneyIdMarkersGet(
          ...
          // User might not have an active journey, which is fine
          console.log("No active journey found for user:", journeyError);
        } finally {
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                      <span className="flex items-center gap-1">
                        ...
                        {journeyMarkers.length} point{journeyMarkers.length !== 1 ? 's' : ''}
                      </span>
                      <span className="flex items-center gap-1">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
